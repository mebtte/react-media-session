...
const audioStyle = { width: '100%', margin: '20px 0' };
...
const App = () => {
  ...
  const onNextTrack = useCallback(() => setIndex((i) => (i + 1) % musicList.length), []);
  ...
  return (
    ...
    >
      <div className="music-list">
        {musicList.map((m, i) => (
          <Music key={m.id} music={m} playing={i === index} onPlay={() => setIndex(i)} />
        ))}
      </div>
      <audio src={music.src} controls autoPlay loop ref={audioRef} style={audioStyle} />
    </MediaSession>
  );
};
